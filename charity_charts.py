...
def create_state_map(df, field_name, title, statistic, color_scheme):
    ...
    lyt = dict(geo = dict(scope = 'usa'), title_text = 'Map of ' + title + ' by State')
    map_ = go.Figure(data=[trc], layout=lyt)
    plot(map_)

def plot_distribution(df, field_name, title, stack_field=None, stack_title='', log_x=False):
    ...
    if stack_field is not None:
        ...
        fig = px.histogram(df_not_missing, x=field_name, color=stack_field,
                           ...
                           labels={field_name: title, stack_field: stack_title})
    else:
        fig = px.histogram(df_not_missing, x=field_name, title='Histogram of ' + title,
                           labels={field_name: title}, log_x=log_x)
    plot(fig)

def plot_relationship(df, x, y, x_title, y_title, log_x=False, log_y=False):
    ...
    fig = px.scatter(df_not_missing, x=x, y=y, title='Relationship Between ' + x_title + ' and ' + y_title,
                     labels={x: x_title, y: y_title}, log_x=log_x, log_y=log_y)
    plot(fig)